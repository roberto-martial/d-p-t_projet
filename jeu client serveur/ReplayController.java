...
public class ReplayController extends ControllerBase {
    ...
    /**
     * Navigue vers l'étape précédente du rapport.
     *
     * Cette méthode permet de revenir à l'étape précédente dans le rapport en
     * ajustant l'index courant. Si l'utilisateur essaie de revenir avant la première
     * étape, l'index est bloqué sur 0 et un message d'avertissement est affiché.
     */
    private void goBackward() {
        ...
        if (currentIndex < 0) {
            System.out.println("on ne peut pas aller plus en arriere");
            currentIndex = 0;
        } else {
            showCurrentMessageAndImage();
        }
    ...
    /**
     * Navigue vers la rencontre suivante dans le replay.
     *
     * Cette méthode permet de passer à l'étape suivante dans le rapport en
     * ajustant l'index courant. Si l'utilisateur essaie d'avancer au-delà de la
     * dernière étape, l'index est bloqué sur la dernière étape et un message
    ...
    private void goForward() {
        ...
        currentIndex = currentIndex+1;
            if (currentIndex >= encounters.length) {
                System.out.println("on ne peut pas aller plus en avant");
                currentIndex = encounters.length -1;
            }else{
                showCurrentMessageAndImage();
            }
    }
